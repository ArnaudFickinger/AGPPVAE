...
#                 pl.ylim(0.0, 0.01)
        ...
        z = Variable(torch.randn(4, 256), requires_grad=False)
        z=z.to(device)
        # Generate a batch of images
        gen_imgs = vae.decode(z).data.cpu().numpy().transpose((0, 2, 3, 1))
        gen_imgs = np.hstack(gen_imgs)
...
def callback_avae(epoch, val_queue, vae, history, figname, device):
    ...
    with torch.no_grad():
        ...
        # plot history
        xs = sp.arange(1, epoch + 2)
        keys = ["kld", "mse", "abs", "loss_dis", "loss_enc", "loss_dec"]
        plots = [1, 2, 5, 6, 9, 10]
        for ik, key in enumerate(keys):
            pl.subplot(4, 4, plots[ik])
            pl.title(key)
